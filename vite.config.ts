...
export default defineConfig(({ command, mode }) => {
  // Use '/' for local development, '/iKid/' for GitHub Pages (matches repository name exactly)
    ...
    ...(isDev ? [] : [VitePWA({
      ...
      manifest: {
        ...
        icons: [
          ...
          {
            src: base === '/' ? '/apple-touch-icon.png' : `${base}apple-touch-icon.png`,
            sizes: '180x180',
            type: 'image/png',
          },
        ],
        shortcuts: [
          {
            ...
            url: base === '/' ? '/checkin' : `${base}checkin`,
            icons: [{ src: base === '/' ? '/icon-192.png' : `${base}icon-192.png`, sizes: '192x192' }],
          },
          {
            name: 'Kryss ut',
            short_name: 'Ut',
            description: 'Kryss ut et barn',
            url: base === '/' ? '/checkout' : `${base}checkout`,
            icons: [{ src: base === '/' ? '/icon-192.png' : `${base}icon-192.png`, sizes: '192x192' }],
          },
        ],
      },
